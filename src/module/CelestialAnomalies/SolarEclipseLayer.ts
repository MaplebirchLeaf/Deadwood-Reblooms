// ./src/module/CelestialAnomalies/SolarEclipseLayer.ts
...
function eclipseStrength(): number {
  const progress = Math.clamp(maplebirch.CA.Phase, 0, 1) * 25;
...
function applySolarEclipseLayer(): void {
  ...
  weather.addLayer(
    'sun',
    {
      effects: Array.from({ length: 7 }, (_, index) => ({
        effect: 'skyOrbital',
        drawCondition(this: any): boolean {
          return Weather.solarEclipse && this.renderInstance.orbitals.sun.factor > -0.5 && !this.renderInstance.sidebarSkyDisabled && maplebirch.CA.StageIndex === index;
        },
        params: {
          images: {
  ...
  // 9. 降水层。新版降水主要由 particleRain / particleSnow 控制颜色。
  weather.addLayer(
    'bannerPrecipitation',
    {
      effects: [
